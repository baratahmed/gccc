...
import { useTopic,useNotification, useAuth } from "@/stores";
...
onMounted(async()=>{
  pinia_topic.loading = true
  const resData = await pinia_topic.show(route.params?.id)
  form.id = resData.data.id
  form.group = resData.data.group
  form.section = resData.data.section
  form.shift = resData.data.shift
  form.session = resData.data.session
  form.due_date = resData.data.due_date
  form.topic = resData.data.topic
  form.instructions = resData.data.instructions
  form.points = resData.data.points
...
const updateTopic = async() => {
  ...
  const resData = await pinia_topic.update(form);
