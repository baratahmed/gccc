...
import { useTopic, useBulkDelete, useAuth } from "@/stores";
...
onMounted(async()=>{
    ...
    await pinia_topic.index(null, null, null)
})
...
const getResults = async (page = 1) => {
    await pinia_topic.index(page, perPage.value, searchQuery.value)
}

// Multiple Delete
const selectAllData = async () => {
    await pinia_bulk_delete.selectAllData(pinia_topic.getItems?.data);
}
...
const multipleDeleteSubmit = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const resData = await pinia_topic.multipleDelete(pinia_bulk_delete.getSelectedData);
...
const singleDelete = (id) => {
  Swal.fire({
      ...
      if (result.isConfirmed) {
        const resData = await pinia_topic.destroy(id);
        if(resData?.status){
            Swal.fire({
            title: "Deleted!",
            text: "Your file has been deleted.",
...
const goToTopicEditPage = (topic) => {
    router.push({name: 'topic.edit', params: {id: topic?.id}});
}

const goToMarkingAssignmentPage = (topic) => {
  router.push({name: 'topic.assignments.marking', params: {id: topic?.id}});
}
            ...
            <div class="card" v-else>
              ...
              <div class="card-body">
                <div class="d-flex">
                    <div class="mr-auto">
                        <div class="tb_search">
                            <input type="text" @input="debounceSearch" v-model="searchQuery" placeholder="Search by topic..." class="form-control">
                        </div>
                    </div>        
                ...
                <div class="d-flex justify-content-first mb-2">
                  ...
                  <div v-if="pinia_bulk_delete.selectedData.length" class="pb-3 mr-2">
                    ...
                    <div class="pb-3" v-if="auth.can('topic.create')">
                        <router-link :to="{name: 'topic.add'}" class="btn btn-sm btn-primary py-2">Add Topic (+)</router-link>
                    </div>
                </div>
                ...
                <Table v-if="pinia_topic.getItems?.data?.length > 0">
                    <template #tableHead>
                        <TableHead v-if="auth.can('topic.delete')">
                        ...
                        <TableHead v-if="auth.can('topic.update') || auth.can('topic.delete')">Action</TableHead>
                    </template>
                    <TableRow v-for="(topic,index) in pinia_topic.getItems?.data" :key="index">
                        <TableData v-if="auth.can('topic.delete')">
                            <input type="checkbox" class="check_box" :checked="pinia_bulk_delete.selectAll" @change="toggleSelectSelection(topic)">
                        </TableData>
                        <TableData>{{topic.topic}}</TableData>
                        <TableData>{{topic.section}}</TableData>
                        <TableData>{{topic.shift}}</TableData>
                        <TableData>{{onlyDateFormat(topic.due_date)}}</TableData>
                        <TableData>{{topic.total_assignments}}</TableData>                       

                        <TableData v-if="auth.can('topic.update') || auth.can('topic.delete')">
                            <div class="d-flex" style="gap:4px">
                                <SuccessButton type="button" @click.prevent="goToMarkingAssignmentPage(topic)" v-if="auth.can('assignment.marking')" title="Start Marking">
                                    <i class="fa fa-check"></i>
                                </SuccessButton>
                                <PrimaryButton type="button" @click.prevent="goToTopicEditPage(topic)" v-if="auth.can('topic.update')">
