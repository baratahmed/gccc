...
onMounted(async()=>{
    ...
    await pinia_attendance.index(null, null, null)
})
...
const getResults = async (page = 1) => {
    await pinia_attendance.index(page, perPage.value, searchQuery.value)
}
...
const goToAttendanceEditPage = (attendance) => {
    router.push({name: 'attendance.edit', params: {id: attendance?.id}});
}
              ...
              <div class="card-body">
                ...
                <Table v-if="pinia_attendance.getItems?.data?.length > 0">
                    ...
                    <TableRow v-for="(attendance,index) in pinia_attendance.getItems?.data" :key="index">
                        ...
                        <TableData>{{attendance.section}}</TableData>
                        <TableData>{{attendance.shift}}</TableData>
                        ...
                        <TableData v-if="auth.can('attendance.update') || auth.can('attendance.delete')">
                            <div class="d-flex" style="gap:4px">
                                <PrimaryButton type="button" @click.prevent="goToAttendanceEditPage(attendance)" v-if="auth.can('attendance.update')">
